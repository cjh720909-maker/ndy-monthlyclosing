...
function VehicleAbsencePage() {
    ...
    const handleSearch = async ()=>{
        ...
        for(let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)){
            ...
            // Skip Saturday (6) and Sunday (0) -> Requirement: Mon-Fri check
            // User said "Excluding Sundays... Mon-Sat among... No dispatch on Mon-Fri..."
            // Let's assume Mon-Fri are mandatory working days to check absence.
            if (dayOfWeek === 0 || dayOfWeek === 6) continue;
            const dateStr = formatDate(d);
            // Check each Regular driver
            const regularDrivers = MOCK_DRIVERS.filter((driver)=>driver.type === 'Regular');
            regularDrivers.forEach((driver)=>{
                const hasDispatch = MOCK_DISPATCHES.some((dispatch)=>dispatch.driverId === driver.id && dispatch.date === dateStr);
                if (!hasDispatch) {
                    absences.push({
                        date: dateStr,
...
//# sourceMappingURL=_39ba5151._.js.map